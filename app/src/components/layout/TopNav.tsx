 ...
 * Moduli richiamati: `next/link`, `next-intl`, `@/lib/supabase/client`, `next/navigation`, `@/lib/types/domain`
        ...
        <div className="flex items-center gap-8">
          ...
          {variant === 'admin' && (
            <div className="flex items-center gap-3">
              ...
              <span className="text-label-caps px-2 py-0.5 bg-tertiary text-on-tertiary rounded-full">
                Admin
              </span>
            </div>
          )}
                ...
                <button className="flex items-center gap-2 rounded-full hover:ring-2 hover:ring-primary/30 transition-all">
                  {session.avatarUrl ? (
                    ...
                    <img
                      ...
                      alt={session.displayName ?? 'Profilo'}
                      className="w-8 h-8 rounded-full object-cover border-2 border-white shadow-ambient-md"
                    />
                  ) : (
...
function NavLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className="text-on-surface-variant hover:text-primary font-medium transition-colors text-sm"
    >
      {children}
    </Link>
  )
